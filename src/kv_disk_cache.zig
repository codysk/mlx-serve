...
pub const DiskTier = struct {
    ...
    pub fn appendCommitWithSpec(
        ...
        var ssm_res = self.persistSsmCheckpoints(id, dir_rel, kv_target, old_ssm_pos, old_ssm_bytes, ssm_checkpoints, &written_bytes, self.max_flush_bytes / 2) catch |err| {
        ...
        // positions) is (re)written — up to the REMAINDER of the per-flush
        // byte cap. Stopping early lands on a full-chunk boundary; the entry
        // then records the shorter kv_len and the NEXT flush resumes from
        // there.
    ...
    fn appendSsmOnly(
        ...
        var ssm_res = try self.persistSsmCheckpoints(e.id, dir_rel, @intCast(e.tokens.len), e.ssm_positions, e.ssm_bytes, ssm_checkpoints, &written_bytes, self.max_flush_bytes);
        errdefer ssm_res.deinit(self.allocator);
